...
class ComparePage {
    ...
    populateWebScrapeReport(reportHtml) {
        ...
        this.missingLocalSitesTable = this.missingLocalSitesTable.on('click','td:first-child a',ComparePage.handleMissingSiteClick).css('width', '100%').DataTable({
            ...
            columnDefs: [
                { targets: [6, 9, -1], responsivePriority: 4 },
                { targets: [3, 4], responsivePriority: 3 },
                { targets: [5, 7], responsivePriority: 2 },
                { targets: [10, 11], responsivePriority: 1 },
                { targets: 2, className: 'all never dtr-control' },
                { targets: [0, 1], className: 'all never' },
                { targets: 8, className: 'all' }
            ]
        }).on('responsive-display', e => console.log(e));
        this.missingTeslaSitesTable = this.missingTeslaSitesTable.on('click','td:first-child a',ComparePage.handleExistingSiteClick).css('width', '100%').DataTable({
            order: [[1, 'asc']],
            lengthMenu: [ 10, 25, 100, 1000, 10000],
            ...
            columnDefs: [
                ...
                { targets: 3, className: 'all never dtr-control' },
                { targets: [0, 1, 2], className: 'all never' },
                { targets: [6, 7, 8], className: 'all' }
            ]
        });
    ...
    static handleMissingSiteClick(event) {
        const link = $(event.target);

        if (!link.closest('.child').length) {
